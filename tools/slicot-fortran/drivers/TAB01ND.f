...
*     .. Local Scalars ..
      ...
      INTEGER          I, INFO, INDCON, J, M, N, NCONT, NIN, NOUT
      CHARACTER*1      JOBZ
      CHARACTER*256   INPFIL, OUTFIL
*     .. Local Arrays ..
...
*     Optional file I/O: TAB01ND [input.dat [output.res]]
*     If no args, use stdin (5) / stdout (6). Else open input and/or output files.
      NIN = 5
      NOUT = 6
      IF ( IARGC() .GE. 1 ) THEN
         CALL GETARG( 1, INPFIL )
         OPEN( UNIT = 10, FILE = INPFIL, STATUS = 'OLD', ERR = 99 )
         NIN = 10
      END IF
      IF ( IARGC() .GE. 2 ) THEN
         CALL GETARG( 2, OUTFIL )
         OPEN( UNIT = 11, FILE = OUTFIL, STATUS = 'REPLACE', ERR = 98 )
         NOUT = 11
      END IF
*
...
*
   ...
   98 CONTINUE
      ...
      STOP
   99 CONTINUE
      WRITE ( 6, FMT = '('' Error opening input file'')' )
      STOP
*
