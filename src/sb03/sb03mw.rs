//! SB03MW — Solve 2×2 continuous Lyapunov: op(T)'*X + X*op(T) = scale*B (symmetric X).
...
/// Solves op(T)'*X + X*op(T) = scale*B for symmetric 2×2 X.
/// `ltran`: false = op(T)=T, true = op(T)=T'.
/// `lupper`: true = use upper triangle of B/X, false = lower.
/// Returns (scale, xnorm, info). info=1 if T and -T' had close eigenvalues (perturbed).
...
pub fn sb03mw(
    ...
    *info = 0;
    ...
    // Build 3×3 system: vec(X) = [x11, x12, x22] (upper) or [x11, x21, x22] (lower)
...
mod tests {
    ...
    #[test]
    fn test_sb03mw_1x1_equiv() {
        let t = [2.0, 0.0, 0.0, 2.0];
        let b = [1.0, 0.0, 0.0, 1.0];
        let mut x = [0.0; 4];
        let mut scale = 0.0;
        let mut xnorm = 0.0;
        let mut info = 0;
        sb03mw(false, true, &t, 2, &b, 2, &mut scale, &mut x, 2, &mut xnorm, &mut info);
        assert_eq!(info, 0);
        assert!((x[0] - 0.25).abs() < 1e-10);
        assert!((x[3] - 0.25).abs() < 1e-10);
    }
}
